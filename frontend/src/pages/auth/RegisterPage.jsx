...
const RegisterPage = ({ addToast }) => {
  ...
  useEffect(() => {
    ...
    else if (score >= 3)  setPasswordStrength({ score: 100, label: 'Strong',   color: 'bg-[#00C2A8]' });
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      } else setError(be?.message || 'Something went wrong. Please try again.');
        ...
        <div>
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.15 }} className="max-w-xl">
            <h1 className="text-4xl lg:text-6xl font-bold leading-tight mb-6" style={{ color: dark ? '#fff' : '#002920' }}>
              Build your<br />financial<br />
              <span className="text-transparent bg-clip-text" style={{ backgroundImage: 'linear-gradient(90deg,#00C2A8,#00E0C8)' }}>future.</span>
            </h1>
          ...
          >
            <div style={{ background: cardBg, borderRadius: '1.15rem', padding: '2rem' }}>
              ...
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }} className="mb-6 relative">
                <h2 className="text-2xl font-bold" style={{ color: titleCol }}>Create an account</h2>
                <p className="mt-1 text-sm" style={{ color: subCol }}>Join thousands of Moroccans banking better</p>
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                <motion.div initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.25 }}>
                  <Input label="Full Name" placeholder="Mohamed Alami" leftIcon={User}
                    light={!dark}
                ...
                <motion.div initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.3 }}>
                  <Input label="Email Address" type="email" placeholder="mohamed@alami.ma" leftIcon={Mail}
                    light={!dark}
                ...
                <motion.div initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.35 }} className="relative">
                  <Input label="Password" type={showPassword ? 'text' : 'password'} placeholder="••••••••" leftIcon={Lock}
                    light={!dark}
                  ...
                  {password.length > 0 && (
                    <div className="mt-2 space-y-1">
                      ...
                      <p className="text-[10px] text-right font-medium uppercase tracking-wider" style={{ color: subCol }}>
                        Strength: <span style={{ color: passwordStrength.score === 100 ? '#00C2A8' : undefined }}
                ...
                <motion.div initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.4 }}>
                  <Input label="Confirm Password" type="password" placeholder="••••••••" leftIcon={Lock}
                    light={!dark}
                    error={errors.confirmPassword?.message} disabled={isLoading}
                    {...register('confirmPassword', { required: 'Please confirm your password', validate: val => watch('password') !== val ? 'Passwords do not match' : true })} />
                </motion.div>
                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5, delay: 0.45 }}>
                  <label className="flex items-start gap-2 cursor-pointer pt-1">
                    ...
                    <span className="text-xs leading-relaxed" style={{ color: subCol }}>
                      I agree to the <Link to="/terms" style={{ color: '#00C2A8' }}>Terms of Service</Link> and <Link to="/privacy" style={{ color: '#00C2A8' }}>Privacy Policy</Link>
                    </span>
                  </label>
                ...
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.5 }}>
                  <Button type="submit" className="w-full mt-2" isLoading={isLoading} variant="primary">Create account</Button>
                </motion.div>
              </form>

              <p className="mt-6 text-center text-sm" style={{ color: subCol }}>
                Already have an account?{' '}
                <Link to="/login" className="font-semibold" style={{ color: '#00C2A8' }}>Sign in</Link>
              </p>
            </div>
          </motion.div>
        </div>
