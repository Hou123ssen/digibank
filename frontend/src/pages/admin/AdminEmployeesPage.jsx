  ...
  const handleSubmit = async e => {
    ...
    try {
      await adminService.createEmployee({
        ...
        departments: form.departments, send_invite: form.sendInvite,
      });
        ...
        <form onSubmit={handleSubmit} className="p-7 space-y-5 max-h-[70vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <input
                required placeholder="Mehdi Alami"
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 mt-1.5">
                <span className={cn('flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold border', deptMeta.color)}>
                  <DeptIcon size={10} /> {employee.department}
                </span>
        ...
        <div className="flex-1 overflow-y-auto p-5">
          ...
          {tab === 'performance' && (
            <div className="space-y-5">
              ...
              {[
                { label: 'Tickets Resolved', val: perf?.tickets_resolved ?? 42, max: 60 },
                { label: 'KYC Processed',    val: perf?.kyc_processed    ?? 18, max: 30 },
                { label: 'Avg Response Time',val: perf?.response_time_score ?? 78, max: 100 },
                { label: 'Customer Rating',  val: perf?.customer_rating  ?? 91, max: 100 },
              ].map(m => (
          ...
          <Table
            ...
            data={employees.map(emp => [
              <div className="flex items-center gap-3">
                ...
                />
                <div className="hidden w-9 h-9 rounded-xl bg-violet-500/10 border border-violet-500/20 items-center justify-center text-violet-400 font-bold shrink-0">
                  {emp.name.charAt(0)}
                </div>
